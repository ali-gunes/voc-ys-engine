...
import { fetchRestaurantData, fetchAutocomplete, fetchGraphQLData } from './scraper.js';
...
// Endpoint for restaurant listings based on coordinates (GraphQL)
// Usage: GET /restaurants?lat=41.0627474&lng=28.993111
app.get('/restaurants', async (req, res) => {
    ...
    try {
        const result = await fetchGraphQLData(lat, lng);

        if (result) {
            return res.json(result);
        } else {
            return res.status(404).json({ error: "Failed to fetch restaurant listings." });
        }
    } catch (error) {
        console.error("Error during GraphQL fetch:", error);
        return res.status(500).json({ error: "An internal error occurred during GraphQL fetch." });
    }
});
